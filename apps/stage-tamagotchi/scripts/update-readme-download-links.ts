...
async function main() {
  ...
  function updateContent(content: string): string {
    return content
      .replace(
        GITHUB_WINDOWS_RE,
        `https://github.com/mira-assistant/mira-assistant/releases/download/v${cleanVersion}/${windowsExe}`,
      )
      .replace(
        GITHUB_MACOS_RE,
        `https://github.com/mira-assistant/mira-assistant/releases/download/v${cleanVersion}/${macosDmg}`,
      )
      .replace(
        OSS_WINDOWS_RE,
