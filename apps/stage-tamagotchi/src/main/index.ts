...
import messages from '@proj-mira/i18n/locales'

import { electronApp, optimizer } from '@electron-toolkit/utils'
import { Format, LogLevel, setGlobalFormat, setGlobalHookPostLog, setGlobalLogLevel, useLogg } from '@guiiai/logg'
import { initScreenCaptureForMain } from '@proj-mira/electron-screen-capture/main'
...
import { setupServerChannel } from './services/mira/channel-server'
import { setupMcpStdioManager } from './services/mira/mcp-servers'
import { setupPluginHost } from './services/mira/plugins'
import { setupAutoUpdater } from './services/electron/auto-updater'
import { parseRuntimeCommandsFromEnv, runtimeCoordinator } from './services/local-runtime'
...
if (isLinux) {
  ...
  // Fixes: https://github.com/mira-assistant/desktop/issues/757
...
electronApp.setAppUserModelId('com.mira.assistant.desktop')
...
app.whenReady().then(async () => {
  ...
  const runtimeCommands = parseRuntimeCommandsFromEnv()
  await runtimeCoordinator.start(runtimeCommands)
...
async function handleAppExit() {
  ...
  await Promise.all([
    ...
    logIfError('stop runtime processes', () => runtimeCoordinator.stopAll()),
