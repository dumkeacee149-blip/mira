...
export async function setupMainWindow(params: {
  ...
  const mainWindowConfig = getConfig().windows?.find(w => w.title === BRAND_UI.mainWindowTitle && w.tag === 'main')

  const window = new BrowserWindow({
    title: BRAND_UI.mainWindowTitle,
  ...
  function handleNewBounds(newBounds: Rectangle) {
    ...
    const existingConfigIndex = config.windows.findIndex(w => w.title === BRAND_UI.mainWindowTitle && w.tag === 'main')
    ...
    if (existingConfigIndex === -1) {
      ...
      const mainWindowConfig = defu(config.windows[existingConfigIndex], { title: BRAND_UI.mainWindowTitle, tag: 'main' })
