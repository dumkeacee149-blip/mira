...
export async function setupDashboardWindow(params: {
  ...
  const windowConfig = getConfig().windows?.find(w => w.title === BRAND_UI.dashboardWindowTitle && w.tag === 'dashboard')

  const window = new BrowserWindow({
    title: BRAND_UI.dashboardWindowTitle,
  ...
  function handleNewBounds(newBounds: Rectangle) {
    ...
    const existingConfigIndex = config.windows.findIndex(w => w.title === BRAND_UI.dashboardWindowTitle && w.tag === 'dashboard')
    ...
    if (existingConfigIndex === -1) {
      ...
      const windowConfig = defu(config.windows[existingConfigIndex], { title: BRAND_UI.dashboardWindowTitle, tag: 'dashboard' })
