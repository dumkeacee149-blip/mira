import type { Server, ServerOptions } from '@proj-mira/server-runtime/server'
...
import { createServer, getLocalIPs } from '@proj-mira/server-runtime/server'
...
async function installCACertificate(caCert: string) {
  ...
  try {
    if (platform === 'darwin') {
      ...
      const caFileName = 'mira-websocket-ca.crt'
      try {
