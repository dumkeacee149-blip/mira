...
import type { ServerOptions } from '@proj-mira/server-runtime/server'
import type {
...
export const pluginProtocolListProvidersEventName = 'proj-mira:plugin-sdk:apis:protocol:resources:providers:list-providers'
...
// @proj-mira/plugin-sdk (CapabilityDescriptor) once stage-ui and the shared
...
export interface ElectronRuntimeServiceStatus {
  managed: boolean
  processes: Array<{
    name: string
    startedAt?: number
...
export const electronRuntimeGetStatus = defineInvokeEventa<ElectronRuntimeServiceStatus>('eventa:invoke:electron:runtime:get-status')
export const electronRuntimeRestart = defineInvokeEventa<void, { force?: boolean }>('eventa:invoke:electron:runtime:restart')
...
export { electron } from '@proj-mira/electron-eventa'
export * from '@proj-mira/electron-eventa/electron-updater'
