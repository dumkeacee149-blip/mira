...
export async function activate(context: vscode.ExtensionContext) {
  ...
  useLogger().log('MIRA is activating...')

  // Get the configuration
  const config = workspace.getConfiguration('mira-vscode')
  ...
  const client = injeca.provide('proj-mira:client', () => new Client())
...
async function setup(params: {
  ...
  // Connect to Mira Channel Server
  if (params.isEnabled) {
    const connected = await params.client.connect()
    if (connected) {
      window.showInformationMessage('MIRA Server Channel connected!')
    }
    else {
      window.showWarningMessage('MIRA Server Channel connection failed!')
    }
  }

  // Register commands
  params.vscodeContext.subscriptions.push(
    commands.registerCommand('mira-vscode.enable', async () => {
      params.isEnabled = true
      await params.client.connect()
      await registerListeners({ ...params })
      window.showInformationMessage('MIRA enabled!')
    }),

    commands.registerCommand('mira-vscode.disable', () => {
      ...
      window.showInformationMessage('MIRA disabled!')
    }),

    commands.registerCommand('mira-vscode.status', () => {
      const status = params.isEnabled && params.client ? 'Connected' : 'Disconnected'
      window.showInformationMessage(`MIRA Server Channel status: ${status}.`)
    }),
  )
  ...
  useLogger().log('MIRA activated successfully')
}
...
export async function deactivate() {
  const { client } = await injeca.resolve({ client: { key: 'proj-mira:client' } as unknown as ProvidedBy<Client> })
  ...
  client?.disconnect()
  useLogger().log('MIRA deactivated!')
}
