...
- **Adapters**: MIRA Server SDK, MCP SDK
...
Provides integration with the MIRA LLM platform, handling event-driven communication.
...
async function startMIRAModule() {
  ...
  // Create MIRA adapter
  const miraAdapter = new MIRAAdapter(twitter, {
    url: process.env.MIRA_URL,
    token: process.env.MIRA_TOKEN
  })

  // Start adapter
  await miraAdapter.start()

  console.info('Twitter service running as MIRA module')
}
```
