...
import { Client as ServerChannel } from '@proj-mira/server-sdk'
import { ContextUpdateStrategy } from '@proj-mira/server-shared/types'
...
export class DiscordAdapter {
  private miraClient: ServerChannel
  ...
  constructor(config: DiscordAdapterConfig) {
    ...
    this.miraClient = new ServerChannel({
      name: 'discord',
      possibleEvents: [
        'input:text',
        'input:text:voice',
      ...
      token: config.miraToken,
      url: config.miraUrl,
    })

    this.voiceManager = new VoiceManager(this.discordClient, this.miraClient)
  ...
  private setupEventHandlers(): void {
    ...
    this.miraClient.onEvent('module:configure', async (event) => {
    ...
    this.miraClient.onEvent('input:text', async (event) => {
      log.log('Received input from MIRA system:', event.data.text)
      // Process Discord-related commands
      // For now, we'll just log the input
    })

    // Handle output from MIRA system (IA response)
    this.miraClient.onEvent('output:gen-ai:chat:message', async (event) => {
      try {
