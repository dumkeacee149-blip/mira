import type { MetadataEventSource, WebSocketEvent } from '@proj-mira/server-shared/types'
...
import { MessageHeartbeat, MessageHeartbeatKind, WebSocketEventSource } from '@proj-mira/server-shared/types'
...
export function setupApp(options?: AppOptions): { app: H3, closeAllPeers: () => void } {
  ...
  const appLogger = useLogg('@proj-mira/server-runtime').withLogLevel(logLevelStringToLogLevelMap[appLogLevel]).withFormat(appLogFormat)
  const logger = useLogg('@proj-mira/server-runtime:websocket').withLogLevel(logLevelStringToLogLevelMap[websocketLogLevel]).withFormat(websocketLogFormat)
