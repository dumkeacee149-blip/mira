...
describe('s3', (test) => {
  ...
  describe('S3Provider', () => {
    it('should skip upload', async () => {
      ...
      expect(await s3.shouldSkipUpload(join(cwd(), 'packages', 'stage-ui', 'src', 'assets', 'live2d', 'models', 'hiyori_free_zh.zip'), '/proj-mira/stage-web/assets/hiyori_free_zh-D9UJNK98.zip')).toBe(true)
    })
  })
})
