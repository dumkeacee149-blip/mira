...
import { moduleCompatibilityResult, moduleStatus, registryModulesSync } from '@proj-mira/plugin-protocol/types'
...
describe('for FileSystemPluginHost', () => {
  it('should load test-normal-plugin from manifest', async () => {
    ...
    const pluginDef = await host.loadPluginFor({
      apiVersion: 'v1',
      kind: 'manifest.plugin.mira.local',
      name: 'test-plugin',
      entrypoints: {
  ...
  it('should resolve entrypoint by runtime then default then electron', () => {
    ...
    const baseManifest = {
      ...
      kind: 'manifest.plugin.mira.local' as const,
      name: 'test-plugin',
    }
...
describe('for PluginHost', () => {
  const providersCapability = 'proj-mira:plugin-sdk:apis:protocol:resources:providers:list-providers'
  const testManifest = {
    apiVersion: 'v1' as const,
    kind: 'manifest.plugin.mira.local' as const,
    name: 'test-plugin',
  ...
  it('should be able to expose setupModules', async () => {
    ...
    defineInvokeHandler(ctx, protocolCapabilityWait, async () => {
      return {
        key: 'proj-mira:plugin-sdk:apis:protocol:resources:providers:list-providers',
        state: 'ready',
        updatedAt: Date.now(),
      }
    })
