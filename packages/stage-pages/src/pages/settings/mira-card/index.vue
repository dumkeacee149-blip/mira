...
import type { ccv3 } from '@proj-mira/ccc'

import { Alert } from '@proj-mira/stage-ui/components'
import { useMiraCardStore } from '@proj-mira/stage-ui/stores/modules/mira-card'
import { InputFile } from '@proj-mira/ui'
...
function getModuleShortName(id: string, module: 'consciousness' | 'voice') {
  const card = cards.value.get(id)
  if (!card || !card.extensions?.mira?.modules)
    return 'default'

  const miraExt = card.extensions.mira.modules

  if (module === 'consciousness') {
    return miraExt.consciousness?.model ? miraExt.consciousness.model.split('-').pop() || 'default' : 'default'
  }
  else if (module === 'voice') {
    return miraExt.speech?.voice_id || 'default'
  }
