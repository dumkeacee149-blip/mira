...
import type { Card } from '@proj-mira/ccc'
import type { MiraExtension } from '@proj-mira/stage-ui/stores/modules/mira-card'
...
import { Button, FieldInput, FieldValues } from '@proj-mira/ui'
import { Select } from '@proj-mira/ui/components/form'
...
function initializeCard(): Card {
  ...
  const miraExt = existingCard?.extensions?.mira as MiraExtension | undefined

  // Initialize module selections with fallback logic (handles all cases: create, edit with/without extension)
  selectedConsciousnessProvider.value = miraExt?.modules?.consciousness?.provider || consciousnessProvider.value
  selectedConsciousnessModel.value = miraExt?.modules?.consciousness?.model || defaultConsciousnessModel.value
  selectedSpeechProvider.value = miraExt?.modules?.speech?.provider || speechProvider.value
  selectedSpeechModel.value = miraExt?.modules?.speech?.model || defaultSpeechModel.value
  selectedSpeechVoiceId.value = miraExt?.modules?.speech?.voice_id || defaultSpeechVoiceId.value
  selectedDisplayModelId.value = miraExt?.modules?.displayModelId || defaultDisplayModelId.value
