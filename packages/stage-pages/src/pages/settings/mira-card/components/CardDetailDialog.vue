...
import type { MiraCard } from '@proj-mira/stage-ui/stores/modules/mira-card'
...
const selectedCard = computed<MiraCard | undefined>(() => {
  if (!props.cardId)
    return undefined
...
const moduleSettings = computed(() => {
  if (!selectedCard.value || !selectedCard.value.extensions?.mira?.modules) {
    return {
  ...
  const miraExt = selectedCard.value.extensions.mira.modules
  return {
    consciousnessProvider: miraExt.consciousness?.provider || '',
    consciousness: miraExt.consciousness?.model || '',
    speechProvider: miraExt.speech?.provider || '',
    speech: miraExt.speech?.model || '',
    voice: miraExt.speech?.voice_id || '',
  }
})
