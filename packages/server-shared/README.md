# @proj-mira/server-shared

The shared type definitions for all server-side packages of Project Mira.

## Usage

```shell
ni @proj-mira/server-shared -D # from @antfu/ni, can be installed via `npm i -g @antfu/ni`
pnpm i @proj-mira/server-shared -D
yarn i @proj-mira/server-shared -D
npm i @proj-mira/server-shared -D
```

```typescript
import type { WebSocketEvents } from '@proj-mira/server-shared'
```
