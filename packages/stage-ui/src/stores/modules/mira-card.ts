import type { Card, ccv3 } from '@proj-mira/ccc'
...
export interface MiraExtension {
  modules: {
    consciousness: {
...
export interface MiraCard extends Card {
  extensions: {
    mira: MiraExtension
  } & Card['extensions']
}

export const useMiraCardStore = defineStore('mira-card', () => {
  const { t } = useI18n()

  const cards = useLocalStorageManualReset<Map<string, MiraCard>>('mira-cards', new Map())
  const activeCardId = useLocalStorageManualReset<string>('mira-card-active-id', 'default')
  ...
  const addCard = (card: MiraCard | Card | ccv3.CharacterCardV3) => {
    const newCardId = nanoid()
    cards.value.set(newCardId, newMiraCard(card))
    return newCardId
  }
  ...
  const updateCard = (id: string, updates: MiraCard | Card | ccv3.CharacterCardV3) => {
    ...
    cards.value.set(id, newMiraCard(updatedCard))
    return true
  }
  ...
  function resolveMiraExtension(card: Card | ccv3.CharacterCardV3): MiraExtension {
    // Get existing extension if available
    const existingExtension = ('data' in card
      ? card.data?.extensions?.mira
      : card.extensions?.mira) as MiraExtension
  ...
  function newMiraCard(card: Card | ccv3.CharacterCardV3): MiraCard {
    ...
    if ('data' in card) {
      ...
      return {
        ...
        extensions: {
          mira: resolveMiraExtension(ccv3Card),
    ...
    return {
      ...card,
      extensions: {
        mira: resolveMiraExtension(card),
        ...card.extensions,
      },
    }
  }
  ...
  function initialize() {
    if (cards.value.has('default'))
      return
    cards.value.set('default', newMiraCard({
      name: 'ReLU',
      version: '1.0.0',
  ...
  watch(activeCard, (newCard: MiraCard | undefined) => {
    if (!newCard)
      return

    // TODO: Minecraft Agent, etc
    const extension = resolveMiraExtension(newCard)
    if (!extension)
      return
  ...
  return {
    ...
    currentModels: computed(() => {
      return {
        ...
        displayModelId: stageModelStore.stageModelSelected,
      } satisfies MiraExtension['modules']
    }),
