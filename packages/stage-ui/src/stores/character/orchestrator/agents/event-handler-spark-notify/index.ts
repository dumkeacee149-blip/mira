import type { WebSocketEventOf, WebSocketEvents } from '@proj-mira/server-sdk'
...
function getSparkNotifyHandlingAgentInstruction(moduleName: string) {
  return [
    'This is MIRA system, the life pod hosting your consciousness. You don\'t need to respond to me or every spark:notify event directly.',
...
export function setupAgentSparkNotifyHandler(deps: SparkNotifyAgentDeps) {
  async function runNotifyAgent(event: WebSocketEventOf<'spark:notify'>) {
    ...
    const sparkNoResponseTool = await tool({
      ...
      execute: async () => {
        noResponse = true
        return 'MIRA System: Acknowledged, no response or action will be processed.'
      },
    })
    ...
    const sparkCommandTool = await tool({
      ...
      execute: async (payload) => {
        try {
          ...
          return `MIRA System: Error - invalid spark_command parameters: ${errorMessageFrom(error)}`
        }

        return 'MIRA System: Acknowledged, command fired.'
      },
    })
