...
在上周五（3月7日），我一直在尝试设计和构思 MIRA 舞台 UI 和设置 UI 的新风格，这个想法终于在开发直播结束时灵光一现。
...
在开发过程中，我们发现目前位于 `packages/` 目录下的一些包实际上是独立的包，甚至不在 Project Mira 的工作流程中。

这意味着我们现在可以将这些包移动到其他地方，从而简化主仓库 [mira](https://github.com/dumkeacee149-blip/mira) 的安装体积和构建流程。

> 我们要去哪里？

好问题！我们已经在 GitHub 上注册了 [`@proj-mira`](https://github.com/proj-mira) 作为一个组织，由于许多包和静态应用程序对 Moeru AI 也没有用处，也许我们可以将这些包移动到 [`@proj-mira`](https://github.com/proj-mira)。

所以，我们将一些包和应用程序移动到了 [`@proj-mira`](https://github.com/proj-mira) 组织！你可以查看它们：

- https://github.com/proj-mira/webai-examples：用于制作 WebGPU 和相关内容的演示。
- https://github.com/proj-mira/lobe-icons：[Lobe Icons](https://github.com/lobehub/lobe-icons) 的移植版本，用于 Iconify JSON 和 UnoCSS 使用。
