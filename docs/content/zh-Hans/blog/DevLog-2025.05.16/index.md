...
大家好！我是 [Neko](https://github.com/nekomeowww)，[Project Mira](https://github.com/dumkeacee149-blip/mira) 的发起者！

很抱歉在 Project Mira 的 DevLog 更新上有所延迟，请原谅我们的拖延。

> 在过去的几个月里，我们为 MIRA 写了许多精彩的 DevLog，分享我们的开发进展，在其中我们分享了想法、理念，解释了我们使用的技术、从中获得的艺术灵感...一切的一切。
>
...
在过去的几周里，Project Mira 本身的主要任务有一段时间没有进展，也许我在 2025 年 3 月以来的大规模 UI 重构和发布后有些疲惫。大部分工作都是由社区维护者完成的，
...
> 要使用角色卡，导航到设置页面（应用程序右上角，或在桌面应用程序中悬停齿轮图标），找到并点击"Mira Card"按钮。

<img class="light" :src="CharacterCardMenuLight" alt="提供 Mira Card 菜单按钮的菜单截图" />
<img class="dark" :src="CharacterCardMenuDark" alt="提供 Mira Card 菜单按钮的菜单截图" />

> 这将带你到"Mira Card 编辑器界面"，在那里你可以上传和编辑你的角色卡进行人格定制。

<img class="light" :src="CharacterCardSettingsLight" alt="提供 Mira Card 菜单按钮的菜单截图" />
<img class="dark" :src="CharacterCardSettingsDark" alt="提供 Mira Card 菜单按钮的菜单截图" />
...
它在我们的 UI 组件库中是实时的，你可以在这里玩玩：https://mira.local/ui/#/story/src-components-menu-charactercard-story-vue 。
...
从 4 月 15 日开始，我发现 MIRA 中的 VAD（语音激活检测）、[ASR（即自动语音识别）](https://huggingface.co/tasks/automatic-speech-recognition) 和 [TTS（文本转语音）](https://huggingface.co/tasks/text-to-speech) 都非常复杂且难以使用和理解，在那个时候，我正在与 [@himself65](https://github.com/himself65) 合作改进和测试来自 [Llama Index](https://www.llamaindex.ai/) 的新项目的用例，这是一个帮助处理基于事件的 LLM 流式令牌流和音频字节的库，叫做 [`llama-flow`](https://github.com/run-llama/llama-flow)。

[`llama-flow`](https://github.com/run-llama/llama-flow) 真的很小，而且使用起来类型安全。在没有它的旧时代，我必须手动包装另一个**队列**结构，以及 Vue 的响应式驱动的工作流系统，将许多异步任务链接在一起，以便能够处理数据来驱动 MIRA。

那时我开始实验更多的例子，简化 VAD、ASR、TTS 工作流的演示。

最终，我得到了这个：[WebAI 实时语音聊天示例](https://github.com/proj-mira/webai-example-realtime-voice-chat)，我设法证明了这项工作可以在 Web 浏览器中用一个 300 ~ 500 行的 TypeScript 代码来实现 ChatGPT 语音聊天系统。
...
- [VAD + ASR + LLM 聊天](https://github.com/proj-mira/webai-example-realtime-voice-chat/tree/8462ff6bcb83bb278bce5388d588d2e3e3dd6dae/apps/vad-asr-chat)
- [VAD + ASR + LLM 聊天 + TTS](https://github.com/proj-mira/webai-example-realtime-voice-chat/tree/8462ff6bcb83bb278bce5388d588d2e3e3dd6dae/apps/vad-asr-chat-tts)
...
所以 [@luoling](https://github.com/luoling8192) 也为此做了另一个小演示：[Sherpa ONNX 驱动的 VAD + ASR + LLM 聊天 + TTS](https://github.com/proj-mira/webai-example-realtime-voice-chat/tree/main/apps/sherpa-onnx-demo)

#### xsAI 🤗 Transformers.js 的诞生

由于我们为 VAD、ASR、聊天和 TTS 演示所做的工作，这催生了一个名为 [xsAI 🤗 Transformers.js](https://github.com/proj-mira/xsai-transformers) 的新副项目，它简化了调用 WebGPU 驱动的模型推理和使用 workers 提供服务，同时仍然保持与我们之前成功的项目 [xsAI](https://github.com/moeru-ai/xsai) 的 API 兼容性。
...
我在系统中集成 WebGPU 驱动的背景去除器方面做了一些小实验，你可以在 [https://mira.local/devtools/background-remove](https://mira.local/devtools/background-remove) 这里玩玩。
...
- 新的[教程步骤器](https://mira.local/ui/#/story/src-components-misc-steppers-steppers-story-vue?variantId=src-components-misc-steppers-steppers-story-vue-0)、[文件上传](https://mira.local/ui/#/story/src-components-form-input-inputfile-story-vue?variantId=default) 和 [文本区域](https://mira.local/ui/#/story/src-components-form-textarea-textarea-story-vue?variantId=default) 组件
- 颜色问题
- [排版改进](https://mira.local/ui/#/story/stories-typographysans-story-vue?)

更多故事可以在 [Roadmap v0.5](https://github.com/dumkeacee149-blip/mira/issues/113) 中找到
...
这是我，我与其他参与者分享了 MIRA 项目，我们在那里度过了美好的时光！遇到了许多有才华的开发者、产品设计师和企业家。
...
幻灯片本身是完全开源的，你也可以在这里玩玩：[https://talks.ayaka.io/nekoayaka/2025-05-10-mira-how-we-recreated-it/#/1](https://talks.ayaka.io/nekoayaka/2025-05-10-mira-how-we-recreated-it/#/1)
...
- ReLU 角色建模完成！
- 与几家公司就赞助和合作进行了谈判！
- [Roadmap v0.5](https://github.com/dumkeacee149-blip/mira/issues/113) 完成了 92 个任务
  - UI
    - 加载屏幕和教程模块
