...
date: 2025-07-18
excerpt: |
  在阅读完有关 Factorio 强化学习环境的论文后，我们想和大家分享我们如何计划改进 Factorio agent 项目 `mira-factorio`。
preview-cover:
...
大家好，我是 [@LemonNeko](https://github.com/LemonNekoGH)，MIRA 的维护者之一。

## 回顾

半年前，我第一次尝试写一个可以游玩知名自动化生产模拟经营游戏 [Factorio](https://www.factorio.com/) 的 AI Agent [`mira-factorio`](https://github.com/dumkeacee149-blip/mira-factorio)，并在其中进行了这些实践：
...
读到这里，我开始好奇，他们的评测如此复杂，那么在技术实现上是如何保证可维护性的呢？继续阅读发现，他们的实现方式与 `mira-factorio` 非常相似，但是相对 `mira-factorio` 来说有很多优点：
...
和 FLE 比起来，我们的实现显得相当幼稚，那我们要怎么改进 `mira-factorio` 呢？
...
感谢阅读，如果感兴趣，可以翻阅 FLE 的论文和 [代码](https://github.com/JackHopkins/factorio-learning-environment)，也许我的理解有误，欢迎指正！这次的阅读也许不够深入，但是接下来我在按照我的思路来改进 `mira-factorio` 的时候，会需要反复阅读，在有进展时再更新。
