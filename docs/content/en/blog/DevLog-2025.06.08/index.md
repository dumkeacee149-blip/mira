...
Hello everyone, here's LemonNeko, one of maintainer of MIRA. Today's DevLog is talking about: Let Live2D model of MIRA Tamagotchi to focus position.

大家好，我是 LemonNeko，MIRA 的维护者之一，由我来为大家带来今天的 DevLog：让 MIRA 桌宠的 Live2D 模型可以注视鼠标位置。
...
![](./assets/mira-tamagotchi-focus.gif)
...
The blue box is the screen, the pink box is the MIRA window, the purple arrow is the cursor, we define:

蓝色框是屏幕，粉色是 MIRA 的窗口，紫色箭头是鼠标，我们定义：

- The screen size is: `A x B`<br>屏幕高宽为：`A x B`
- The position of MIRA window is: `(E, F)`<br>MIRA 窗口左上角位置为：`(E, F)`
- The size of MIRA window is: `C x D`<br>MIRA 窗口大小为：`C x D`
- The position of cursor is: `G, H`<br>鼠标位置是：`G, H`

Then the relative position of cursor to window is: `(G - E, H - F)`

那么鼠标的位置在 MIRA 窗口中的位置应当是：`(G - E, H - F)`
...
In this DevLog, we learned how to get the relative position of cursor to window, and how to set the focus point of Live2D model manually. If you want to know more about the implementation details, you can check the [source code](https://github.com/dumkeacee149-blip/mira/pull/194) of this PR.
