...
<script setup>
import miraMcpArch from './assets/mira-mcp-arch.avif'
</script>
...
A week ago, I wrote an MCP server [MIRA-android](https://github.com/LemonNekoGH/MIRA-android) for MIRA to connect to mobile phones, but this was only the first half of enabling MIRA to operate Android phones—MIRA also needed to be able to interact with MCP servers.

Over the past two days, I completed the second half by writing a Tauri plugin [#144](https://github.com/moeru-ai/MIRA/pull/144). Now MIRA can interact with MCP servers and work with all existing MCP servers.

If you're interested, check out these two videos. The first demonstrates MIRA's MCP server setup, and the second shows MIRA interacting with an Android phone.

<details>
  <summary>MIRA's MCP Server Setup</summary>
  <ThemedVideo controls muted src="./assets/mira-mcp-settings.mp4" style="height: 640px;" />
</details>

<details>
  <summary>MIRA Inputting `Hello World` on Phone</summary>
  <ThemedVideo controls muted src="./assets/mira-mcp-input-text.mp4" />
</details>

During development, to clarify my thinking, I drew a diagram showing how LLMs call Android phones:

<img :src="miraMcpArch" alt="MIRA Operating Phone" :style="{ height: '640px', objectFit: 'contain' }" />
...
1. From the demo videos, you can see that in the conversation, I first had MIRA get the tool list, then had it input text. Could we get the tool list during initialization and directly append it to the system prompt?
...
2. Should we allow MIRA to connect to multiple phones simultaneously? Might MIRA want to use multiple phones? ~~Would she want to use them for telecom fraud?~~
3. As you can see, the MIRA repository now has both Tauri applications and Tauri plugins. How should this be managed? How should CI be configured? How to synchronize version numbers between the Rust and JavaScript sides of Tauri plugins?

## Future Plans

- Support image return values, so MIRA can see what's on the phone through visual capabilities like Cursor demonstrated in the [previous DevLog](./DevLog-2025.04.22.md), then decide how to interact.
- Let MIRA learn how to use devices itself? If we have to write separate prompts for each type of device, the workload would be enormous.
- Multi-MCP server support. After all, MCP provides a universal interface that allows MIRA to do all sorts of things—MIRA probably won't be satisfied with just operating phones.
- SSE support, so MIRA in the browser can also use MCP servers.
