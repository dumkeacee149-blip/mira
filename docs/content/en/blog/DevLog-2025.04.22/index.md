...
Two months ago, we ported MIRA's web interface to Electron [#7](https://github.com/dumkeacee149-blip/mira/pull/7) (which has now been refactored using Tauri 🤣 [#90](https://github.com/dumkeacee149-blip/mira/pull/90)), allowing it to appear as a desktop pet on our screens. At the same time, I had the idea of allowing MIRA to use mobile phones, but I kept putting it off.

Last weekend (2025.04.20), I spent some time creating an MCP server demo [mira-android](https://github.com/LemonNekoGH/mira-android) that can interact with ADB, providing MIRA with basic mobile interaction capabilities (in fact, most LLMs can interact with phones through it). Here's a demo video:

<ThemedVideo controls muted src="./assets/cursor-open-settings.mp4" />

I also packaged it as a Docker image and submitted it to the [MCP server list](https://mcp.so/server/mira-android/lemonnekogh). Feel free to try it if you're interested.
...
2. LLM responses have length limits. If the operation is complex, it might need to be completed in steps. Can we automatically notify it after each step is completed to trigger the next step, like in [mira-factorio](https://github.com/dumkeacee149-blip/mira-factorio)?
...
Next, I plan to connect the MIRA desktop pet to the MCP server and see what it wants to do. Maybe it will open Telegram and chat with us, just like ReLU does now, but without using Telegram's API.
