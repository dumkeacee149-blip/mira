...
こんにちは、MIRA のメンテナの一人、LemonNeko です。今日の DevLog では、MIRA デスクトップペットの Live2D モデルがマウスの位置を注視できるようにする方法についてお話しします。
...
![](/en/blog/DevLog-2025.06.08/assets/mira-tamagotchi-focus.gif)
...
青い枠が画面、ピンクが MIRA のウィンドウ、紫の矢印がマウスです。次のように定義します：

- 画面の高さと幅：`A x B`
- MIRA ウィンドウの左上の位置：`(E, F)`
- MIRA ウィンドウのサイズ：`C x D`
- マウスの位置：`G, H`

すると、MIRA ウィンドウ内のマウスの位置は `(G - E, H - F)` になるはずです。
